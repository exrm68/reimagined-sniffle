...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [adultWatchAdCount, setAdultWatchAdCount] = useState('-1');
  const [adultPremiumAdCount, setAdultPremiumAdCount] = useState('0');
  ...
  const [editAdultWatchAdCount, setEditAdultWatchAdCount] = useState('-1');
  const [editAdultPremiumAdCount, setEditAdultPremiumAdCount] = useState('0');
  ...
  const handleAddAdult = async () => {
    ...
    try {
      await addDoc(collection(db, 'adultContent'), {
        ...
        watchAdCount: parseInt(adultWatchAdCount) >= 0 ? parseInt(adultWatchAdCount) : -1,
        premiumWatchAdCount: adultIsPremium ? (parseInt(adultPremiumAdCount) || 0) : 0,
        views: 0,
      ...
      setAdultWatchAdCount('-1');
      setAdultPremiumAdCount('0');
  ...
  const handleEditAdult = (item: AdultBannerItem) => {
    ...
    setEditAdultWatchAdCount(String(item.watchAdCount !== undefined ? item.watchAdCount : -1));
    setEditAdultPremiumAdCount(String(item.premiumWatchAdCount || 0));
  };
  ...
  const handleUpdateAdult = async (id: string) => {
    ...
    try {
      await updateDoc(doc(db, 'adultContent', id), {
        ...
        watchAdCount: parseInt(editAdultWatchAdCount) >= 0 ? parseInt(editAdultWatchAdCount) : -1,
        premiumWatchAdCount: editAdultIsPremium ? (parseInt(editAdultPremiumAdCount) || 0) : 0,
      });
          ...
          {activeTab === 'adult' && (
            <div className="space-y-6">
              ...
              <div className="bg-[#18181b] rounded-2xl p-4 space-y-3 border border-white/5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-zinc-400 mb-1 block">Ads count (-1 = global)</label>
                    <input type="number" value={adultWatchAdCount} onChange={e => setAdultWatchAdCount(e.target.value)}
                      placeholder="-1" className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                  </div>
                  {adultIsPremium && (
                    <div>
                      <label className="text-xs text-zinc-400 mb-1 block">Pay করার পর Ads</label>
                      <input type="number" value={adultPremiumAdCount} onChange={e => setAdultPremiumAdCount(e.target.value)}
                        placeholder="0" className="w-full bg-[#242427] border border-amber-500/30 rounded-xl px-3 py-2 text-white text-sm outline-none" />
                    </div>
                  )}
                </div>
                        ...
                        {editingAdultId === item.id ? (
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-1.5 mt-1">
                              <div>
                                <label className="text-[10px] text-zinc-500 block mb-0.5">Ads (-1=global)</label>
                                <input type="number" value={editAdultWatchAdCount} onChange={e => setEditAdultWatchAdCount(e.target.value)}
                                  className="w-full bg-[#111] border border-white/10 rounded-lg px-2 py-1.5 text-white text-xs outline-none" />
                              </div>
                              {editAdultIsPremium && (
                                <div>
                                  <label className="text-[10px] text-zinc-500 block mb-0.5">Post-pay Ads</label>
                                  <input type="number" value={editAdultPremiumAdCount} onChange={e => setEditAdultPremiumAdCount(e.target.value)}
                                    className="w-full bg-[#111] border border-amber-500/20 rounded-lg px-2 py-1.5 text-white text-xs outline-none" />
                                </div>
                              )}
                            </div>
                          ...
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {item.watchAdCount !== undefined && item.watchAdCount >= 0 && <span className="text-[10px] text-blue-400">📺 {item.watchAdCount} ads</span>}
                              {item.isPremium && (item as any).premiumWatchAdCount > 0 && <span className="text-[10px] text-orange-400">+{(item as any).premiumWatchAdCount} post-pay</span>}
                              {(item as any).duration && <span className="text-[10px] text-zinc-500">⏱ {(item as any).duration}</span>}
                              <span className="text-[10px] text-zinc-600">👁 {(item as any).views || 0}</span>
                            </div>
